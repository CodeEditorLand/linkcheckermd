...
let configuration: WorkspaceConfiguration =
	workspace.getConfiguration(configSection);
	...
	getLinks(document)
		.then((links) => {
			if (configuration.get("enableCountryCodeCheck", true)) {
				...
				let countryCodePromise = Promise.all<Diagnostic>(
					links.map((link): Diagnostic => {
				...
				countryCodePromise.then((countryCodeDiag) => {
					...
					let filteredDiag = countryCodeDiag.filter(
	...
	getLinks(document).then((links) => {
		...
		links.forEach((link) => {
			...
			if (isHttpLink(link.address)) {
				...
				brokenLink(link.address, { allowRedirects: true }).then(
					(answer) => {
						// Also check for country code
						if (
							configuration.get("enableCountryCodeCheck", true) &&
							hasCountryCode(link.address)
						) {
							outputChannel.appendLine(
								`Warning: ${link.address} on line ${lineNumber} contains a language reference code.`,
							);
						}
						// Log to the outputChannel
						if (answer) {
							outputChannel.appendLine(
								`Error: ${link.address} on line ${lineNumber} is unreachable.`,
							);
						} else {
							outputChannel.appendLine(
								`Info: ${link.address} on line ${lineNumber}.`,
							);
						}
					},
				);
				...
				if (isFtpLink(link.address)) {
					...
					try {
						...
						if (fs.existsSync(fullPath)) {
							...
							outputChannel.appendLine(
								`Error: ${link.address} on line ${lineNumber} does not exist.`,
							);
						}
					} catch (error) {
						// If there's an error, log the link
						outputChannel.appendLine(
							`Error: ${link.address} on line ${lineNumber} is not an HTTP/s or relative link.`,
						);
					}
				}
			}
...
function isHttpLink(linkToCheck: string): boolean {
	...
	return validator.isURL(linkToCheck, {
